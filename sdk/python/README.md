...
First, clone this repo with the name of the desired provider in place of `influxdb`:

```
git clone https://github.com/pulumi/pulumi-tf-provider-boilerplate pulumi-influxdb
```

Second, replace references to `influxdb` with the name of your provider:
...
- `GO111MODULE=on go get github.com/terraform-providers/terraform-provider-influxdb` (where `influxdb` is the name of the provider)
...
To use from Go, use `go get` to grab the latest version of the library
    ...
    $ go get github.com/pulumi/pulumi-influxdb/sdk/go/...

## Configuration

The following configuration points are available for the `influxdb` provider:

- `influxdb:apiKey` (environment: `XYZ_API_KEY`) - the API key for `influxdb`
- `influxdb:region` (environment: `XYZ_REGION`) - the region in which to deploy resources
